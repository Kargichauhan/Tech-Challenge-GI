"""
Coarse grid-based reachability and route planning.
...
a rendering/physics detail, not a reachability-graph one, and that's a
...
def _resolve_start_cell(grid: GridWorld, cell: tuple, solid_now: set) -> tuple:
    ...
    ground first for this transient case, then fall back to the normal
...
def _simulate_jump(grid: GridWorld, from_cell: tuple, direction: int, solid_now: set, held_keys: frozenset = None):
    ...
    expands, so the same (from_cell, direction) simulation gets re-run
...
def find_path(grid: GridWorld, start_cell: tuple, held_keys: set, target_id: str):
    ...
    Returns a list of (cell, edge_type) pairs, where edge_type is the edge
    used to reach that cell: "start" for the first entry, else "walk"/"fall"/
