...
real genuine_ticks instead: how much longer the honest solve actually took
than a rough optimal-play estimate says it should have. Where the Q-learner
(solvers/q_learner.py) applies, its converged episode length is the more
classically faithful "agent return" and can be substituted in directly; see
compute_regret's `agent_ticks` parameter.

oracle_par_ticks deliberately does not run the genuine solver a second time,
since that's too expensive to call once per candidate scene in an explore
loop that evaluates many candidates per round. Instead it reuses the exact
same planning step GenuineSolver already does
(generation.pathfinding.build_grid plus find_path, over the identical target
sequence solvers.genuine._extract_targets computes) and converts each
returned edge into a closed-form tick estimate from harness.constants,
without ever stepping the pymunk engine.
"""
...
# Frontier band for compute_regret's output, deliberately a separate check
# from archive.compute_fitness, not derived from it. Fitness decides who
# occupies an archive cell (diversity: every valid candidate is considered,
# unconditionally). This band decides whether a candidate is worth the
# invention loop's attention next round (curriculum: only genuinely
# frontier-appropriate scenes qualify). Conflating the two into one score
# meant "goes in the archive" and "is interesting to build on further" were
# never actually distinguishable from the report, since a candidate could
# win a cell on fitness alone without ever being flagged as something worth
# targeting again. Splitting them costs nothing, since both reuse the same
# regret/severity inputs, and makes the loop's own reasoning legible: a
# round's report can now say why a candidate mattered, not just that it
# scored well.
